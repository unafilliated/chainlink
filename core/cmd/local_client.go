...
func (cli *Client) RunNode(c *clipkg.Context) error {
	...
	bootLgr := logger.Default.Named("boot")
	...
	bootLgr.With("Version", static.Version, "SHA", static.Sha, "InstanceUUID", static.InstanceUUID).
		Infof("Starting Chainlink Node %s at commit %s", static.Version, static.Sha)
	if cli.Config.Dev() {
		bootLgr.Warn("Chainlink is running in DEVELOPMENT mode. This is a security risk if enabled in production.")
	}
	if cli.Config.EthereumDisabled() {
		bootLgr.Warn("Ethereum is disabled. Chainlink will only run services that can operate without an ethereum connection")
	}
	...
	for _, ch := range chainSet.Chains() {
		...
		if !fexisted {
			bootLgr.Infow("New funding address created", "address", fkey.Address.Hex(), "evmChainID", ch.ID())
		}
		if !sexisted {
			bootLgr.Infow("New sending address created", "address", skey.Address.Hex(), "evmChainID", ch.ID())
		}
	}
	...
	if !didExist {
		bootLgr.Infof("Created OCR key with ID %s", ocrKey.ID())
	}
	...
	if !didExist {
		bootLgr.Infof("Created P2P key with ID %s", p2pKey.ID())
	}
	...
	bootLgr.Info("API exposed for user ", user.Email)
	...
	bootLgr.Infof("Chainlink booted in %s", time.Since(static.InitTime))
